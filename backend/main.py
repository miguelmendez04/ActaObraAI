...
from backend.auth import (
    ...
    authenticate_user, create_access_token, get_user, User,
    init_db, get_db_connection, get_password_hash
)

# Inicialización automática de Base de Datos para entornos efímeros (como Hugging Face Docker)
def _auto_init_db():
    init_db()
    conn = get_db_connection()
    c = conn.cursor()
    for u, p, cid in [("empresaA", get_password_hash("1234"), "company_A"), ("empresaB", get_password_hash("1234"), "company_B")]:
        try:
            c.execute("INSERT INTO users (username, hashed_password, company_id) VALUES (?, ?, ?)", (u, p, cid))
        except:
            pass
    conn.commit()
    conn.close()
...
async def get_current_user(
    ...
    if user is None:
        raise credentials_exception
    return User(username=user.username, company_id=user.company_id)
...
# Se elimina la instanciación global de 'collection'. Ahora se genera dinámicamente según el company_id del usuario.
...
def get_documents(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        results = user_collection.get(include=["metadatas"])
...
def reset_documents(current_user: User = Depends(get_current_user)):
    """
    Vacía toda la base de datos exclusiva para la empresa del autor.
    """
    try:
        col_name = f"actas_{current_user.company_id}"
        chroma_client.delete_collection(name=col_name)
        # Recrear vacía
        chroma_client.get_or_create_collection(name=col_name, metadata={"hnsw:space": "cosine"})
        return {"status": "success", "message": f"BD de la empresa {current_user.company_id} reiniciada."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def ask(body: AskRequest, current_user: User = Depends(get_current_user)):
    """
    Recibe una pregunta, busca fragmentos relevantes en ChromaDB de la empresa del auth token.
    """
    ...
    # 1. Búsqueda de similitud en ChromaDB (Extendido a 40 fragmentos para cobertura profunda)
    n_results = min(40, max(user_collection.count(), 1))
    results = user_collection.query(query_texts=[question], n_results=n_results)
